...
Then enables the **ALB Controller** add-on (`az aks addon enable --addon alb-controller`) which is the in-cluster component that programs AGC.
...
| Gateway `ADDRESS` stays empty | ALB Controller add-on disabled or pods not Ready | `kubectl get pods -n azure-alb-system` — pods should be `Running`. Re-run `az aks addon enable -n $CLUSTER -g $RG --addon alb-controller` |
